...
HttpSession.prototype.messageLinkedSession = function(msg, cb){
  logger.log('info', 'messaging linked session ' + this.linker.sessid);
  this.linker.messageLinkedSessions(msg);
  if(cb)
    cb();
}

HttpSession.prototype.linkSession = function(linker){
  this.linker = linker;
  this.linker.on('fromLinked', this.messageFromLinkedSession);
}
...
HttpSession.prototype.addMessage = function(message) {
      console.log('sending message  to linker' + this.sessid)
      console.log(message);
