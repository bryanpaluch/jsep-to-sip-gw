...
Gateway.prototype.sendToSipGW= function(msg, sess) {
  ...
  this.sessions[sess].id = [invite.headers['call-id'], invite.headers.to.params.tag, invite.headers.from.params.tag].join(':');
  sip.send(invite,
	function(rs) {
		if (rs.status >= 300) {
			console.log('call failed with status ' + rs.status + ' session= ' + session);
		}
		else if (rs.status < 200) {
			console.log('Call progress status ' + rs.status + ' session= ' + session);
		}
		else if (rs.status == 200){
      console.log('Call answered with status ' + rs.status +' with tag ' + rs.headers.to.params.tag + 'session= ' + session);
      ...
      setTimeout(function(){self.emit(session, {type: 'candidates', candidates:jsepAnswer.candidates});}, 100);
      sip.send({
				method: 'ACK',
...
Gateway.prototype.EndJSEPSession = function(sess){ 
