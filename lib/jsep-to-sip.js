...
Gateway.prototype.AddJSEPMessage = function(sess, message) {
	...
	if (this.sessions[sess]) {
		if (message.sdp) {
			...
			if (message.last) this.sendToSipGW(this.sessions[sess].msg, sess);
  ...
  sip.send({
	...
	function(rs) {
		if (rs.status >= 300) {
			...
			console.log('Call answered with tag ' + rs.headers.to.params.tag + 'session= ' + session);
	    var sipTrunksdp = new sdpParser(rs.content, {type : 'asterisk'});
      var jsepAnswer =  sipTrunksdp.toJSEP();
      self.emit(session, {type: 'answer', sdp:jsepAnswer.sdp});
