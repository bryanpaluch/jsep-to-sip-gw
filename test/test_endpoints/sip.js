...
function SipEndpoint(opts){
  ...
  this.server = null; 
  this.helper = new SipHelper({outboundproxy : opts.outboundproxy, localhost: opts.localhost}); 
...
SipEndpoint.prototype.handleResponse = function(rs){
  if(rs.headers.cseq.method === "INVITE"){
    if(rs.status >= 300){
      ...
      this.reportAnswer(rs.headers);
      var ack = this.helper.createAck(rs);
...
SipEndpoint.prototype.call = function( to, from){
  var self = this; 
  var invite = this.helper.createInvite(this.to, this.from, "rtcgateway", video_audio_offer_sdp)
  this.server.send(invite, this.handleResponse.bind(this));
